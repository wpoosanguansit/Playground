...
import Common._
import com.playground.strategy.Default.processInputs
...
class AgentSpec extends Specification {
  ...
  val initial = Env(
    Point(0,1),
    Set(Point(0,0), Point(2,0), Point(1,1), Point(2,1), Point(1,0)),
    Set(Point(1,1), Point(-1,0), Point(0,1)),
    List[Point](),
    List('B', 'C', 'D', 'E', 'F'),
    Some(Point(1,2)),
    true
  ...
  def shouldHaveFoundH = {
    val state      = processInputs("G", "B", "#", "#", "F").run(initial)
    println(initial)
    val result     = state._1
    println(result)
    val state1     = processInputs("B", "#", "G", "A", "C").run(result)
    val result1    = state1._1
    println(result1)
    val state2     = processInputs("A", "H", "#", "#", "B").run(result1)
    val result2    = state2._1
    println(result2)
    val state3     = processInputs("H", "#", "A", "#", "#").run(result2)
    val result3    = state3._1
    println(result3)
    result3.result must_== List('B', 'C', 'D', 'E', 'F', 'G', 'A', 'H')
  }
